import { Phone, Clock, Mail } from "lucide-react";

const messages = [
  { icon: Phone, label: "24/7 Hotline", value: "800 4 NADZ / 800 4 6239" },
  { icon: Mail, label: "Email", value: "info@nadzhealthcare.com" },
  { icon: Clock, label: "Office Hours", value: "9AM-6PM | Monday – Saturday" },
];

function MarqueeGroup() {
  return (
    <div className="flex shrink-0 items-center gap-10 pr-10">
      {messages.map((m, i) => {
        const Icon = m.icon;
        return (
          <span key={i} className="flex items-center gap-2.5 whitespace-nowrap">
            <span className="grid h-6 w-6 place-items-center rounded-full bg-white/15">
              <Icon className="h-3.5 w-3.5 text-[color:var(--gold-light)]" />
            </span>
            <span className="text-[13px] font-medium text-white/70">
              {m.label}
            </span>
            <span className="text-[13px] font-semibold text-white">
              {m.value}
            </span>
          </span>
        );
      })}
    </div>
  );
}

const socials = [
  {
    name: "X",
    href: "https://x.com/nadzhealthcare",
    path: "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231 5.45-6.231Zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77Z",
  },
  {
    name: "Instagram",
    href: "https://www.instagram.com/nadzhealthcare/",
    path: "M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069ZM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0Zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324ZM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881Z",
  },
  {
    name: "Facebook",
    href: "https://www.facebook.com/nadzhealthcare/",
    path: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073Z",
  },
  {
    name: "LinkedIn",
    href: "https://www.linkedin.com/company/nadzhealthcare/",
    path: "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286ZM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065Zm1.782 13.019H3.555V9h3.564v11.452ZM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0Z",
  },
];

export default function TopBar() {
  return (
    <div className="relative z-50 flex h-9 items-center overflow-hidden bg-[color:var(--maroon)] text-white">
      {/* Scrolling info */}
      <div className="marquee-track relative flex-1 overflow-hidden">
        <div className="animate-marquee flex w-max items-center">
          <MarqueeGroup />
          <MarqueeGroup />
          <MarqueeGroup />
          <MarqueeGroup />
        </div>
      </div>

      {/* Social icons, pinned right */}
      <div className="hidden shrink-0 items-center gap-3 bg-[color:var(--maroon)] pl-6 pr-4 sm:pr-6 md:flex">
        {socials.map((s) => (
          <a
            key={s.name}
            href={s.href}
            target="_blank"
            rel="noopener noreferrer"
            aria-label={s.name}
            className="grid h-7 w-7 place-items-center rounded-full bg-white/10 text-white/80 transition-colors hover:bg-[color:var(--gold)] hover:text-[color:var(--maroon)]"
          >
            <svg viewBox="0 0 24 24" fill="currentColor" className="h-3.5 w-3.5">
              <path d={s.path} />
            </svg>
          </a>
        ))}
      </div>
    </div>
  );
}
